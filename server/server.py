import threading
import socket
# Module to style and control the terminal/console.
import blessings
...
class Console:
    ...
    def print_new_con(self, addr, name):  # Print new connection alert.
        str_addr = str(addr)
        print(self.terminal.yellow("[NEW CONNECTION]: ") + self.terminal.green(str_addr) +
...
class Server:
    ...
    default_ip = [(s.connect(('8.8.8.8', 53)), s.getsockname()[0],
                   s.close()) for s in [socket.socket(socket.AF_INET,
                                                      socket.SOCK_DGRAM)]][0][1]  # Server's IPv4 address.
    default_port = 5050
    default_max_con = 5
    ...
    def __handle_discon(self, conn, addr, user):  # Handle disconnection of a client.
        self.conn_clients.remove((user, conn))
    ...
    def __handle_client(self, conn, addr):
        ...
        console.print_new_con(self.ADDR, client.user_name)
        self.conn_clients.append((client.user_name, client.CONN))
        ...
        self.__handle_discon(conn, addr, client.user_name)  # When client disconnects.

    def listen_conn(self):  # Listen to new connections and assign a new thread to each one.
